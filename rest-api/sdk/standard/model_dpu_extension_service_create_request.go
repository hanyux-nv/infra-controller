...
type DpuExtensionServiceCreateRequest struct {
	...
	// Deployment spec, limited to 131072 bytes once UTF-8 encoded; characters outside ASCII count as more than one byte. KubernetesPod requires a parseable Pod manifest declaring at least one container, whose kind, when present, is Pod. DpfHelmChart requires a strict JSON object with repoURL, chartName, chartVersion, security.privileged, and optional values and serviceDaemonSet; repoURL must use https:// or oci://. The typed serviceDaemonSet supports only labels, annotations, ResourceList-style resources, and updateStrategy. Metadata uses Kubernetes key/value syntax. updateStrategy supports RollingUpdate and OnDelete. Rolling-update limits are non-negative integers or percentage strings from 0% through 100%; after Kubernetes defaults are applied, exactly one of maxSurge and maxUnavailable must be non-zero. Explicit serviceDaemonSet.nodeSelector, the incorrect upgradeStrategy spelling, values.serviceDaemonSet.nodeSelector, and all other unsupported fields are rejected because NICo owns placement and the typed contract is strict.
